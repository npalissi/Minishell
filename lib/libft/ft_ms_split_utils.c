...
/*   Updated: 2025/02/11 18:12:03 by edubois-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (s[i] && (*idq || *iq || (s[i] != ' ' && s[i + 1] != '|')))
	{
		*w++ = s[i];
		if (s[i] == '"' && *iq == 0 && ((i == 0) || (i > 0 && s[i - 1] != '\\')))
			*idq = 0;
		else if (s[i] == '\'' && *idq == 0 && ((i == 0) || (i > 0 && s[i - 1] != '\\')))
			*iq = 0;
		i++;
	}
	*word = save_w;
...
void add_simple_word(char **word, char **str)
{
	...
	iq[0] = 0;
	iq[1] = 0;
	while (s[i] && !iq[0] && !iq[1] && s[i] != ' ' && ((s[i] == '|' && i > 0 && s[i - 1] == '\\') || s[i] != '|'))
	{
		*w++ = s[i];
	...
	while (s[i] && s[i] != ' ' && ((s[i] == '|' && i > 0 && s[i - 1] == '\\') || s[i] != '|'))
		*w++ = s[i++];
	*word = s_w;
	*str = s + i;
}
